...
const getUserChannelProfile = asyncHandler(async (req, res) => {
  const { username } = req.params; //* USing params since we will get the username from the url

  if (!username?.trim()) {
    throw new ApiError(400, "Invalid username");
  }

  //TODO: writing the aggregation pipeline
  ...
  const channel = await User.aggregate([
    ...
    {
      //? addFields lets you add additional fields
      $addFields: {
        subscribersCount: {
          $size: "$subscribers",
        },
